...
class LinkButtonModal(discord.ui.Modal, title="Add a Link Button"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if total >= 5:
            embed = discord.Embed(description="Max 5 buttons total.", color=discord.Color.from_rgb(255, 255, 255))
            await interaction.response.send_message(embed=embed, ephemeral=True)
            return

        self.builder.link_buttons.append((self.label.value, self.url.value))
        confirm = discord.Embed(
            description=f"🔗 Link button **{self.label.value}** added.",
...
class ButtonTypeView(discord.ui.View):
    ...
    @discord.ui.button(label="Message", style=discord.ButtonStyle.secondary, emoji="💬")
    ...
    async def cancel(self, interaction: discord.Interaction, button: discord.ui.Button):
        embed = discord.Embed(description="Cancelled.", color=discord.Color.from_rgb(255, 255, 255))
        await interaction.response.edit_message(embed=embed, view=None)
...
class MessageButtonSubBuilder(discord.ui.View):
    ...
    def __init__(self, parent: "EmbedBuilderView", label: str):
        ...
        self.embed = discord.Embed(
            title=f"💬 Configuring: {label}",
            description="Set this button's message content below.",
            color=discord.Color.from_rgb(255, 255, 255),
        )
        self.ephemeral = True
    ...
    @discord.ui.button(label="Title", style=discord.ButtonStyle.secondary, emoji="📝", row=0)
    async def set_title(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(SubTitleModal(self))

    @discord.ui.button(label="Description", style=discord.ButtonStyle.secondary, emoji="📄", row=0)
    async def set_description(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(SubDescriptionModal(self))

    @discord.ui.button(label="Banner", style=discord.ButtonStyle.secondary, emoji="🖼️", row=0)
    ...
    @discord.ui.button(label="Ephemeral: Yes", style=discord.ButtonStyle.secondary, emoji="👁️", row=1)
    ...
    @discord.ui.button(label="Save", style=discord.ButtonStyle.secondary, emoji="✅", row=2)
    async def save(self, interaction: discord.Interaction, button: discord.ui.Button):
        self.parent.message_buttons.append(
            {"label": self.label, "embed": self.embed.copy(), "ephemeral": self.ephemeral}
        )
        confirm = discord.Embed(
            description=f"💬 Message button **{self.label}** saved.", color=discord.Color.from_rgb(255, 255, 255)
        )
        await interaction.response.edit_message(embed=confirm, view=None)
...
class EmbedBuilderView(discord.ui.View):
    def __init__(self, author_id: int, default_channel: discord.TextChannel = None):
        ...
        self.embed = discord.Embed(
            title="Your Embed",
            description="*Nothing set yet — use the buttons above to start building.*",
            color=discord.Color.from_rgb(255, 255, 255),
        )
    ...
    async def update_preview(self):
        ...
        dest = f"#{self.target_channel.name}" if self.target_channel else "No channel selected"
        base_footer = preview.footer.text
        preview.set_footer(text=f"{base_footer}  •  📍 {dest}" if base_footer else f"📍 {dest}")
    ...
    @discord.ui.button(label="Footer", style=discord.ButtonStyle.secondary, emoji="🏷️", row=0)
    ...
    @discord.ui.button(label="Add Button", style=discord.ButtonStyle.secondary, emoji="🔘", row=1)
    async def add_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        embed = discord.Embed(
            description="Choose the button type you'd like to add.",
            color=discord.Color.from_rgb(255, 255, 255),
        )
    ...
    @discord.ui.button(label="Send", style=discord.ButtonStyle.secondary, emoji="✅", row=3)
    async def send(self, interaction: discord.Interaction, button: discord.ui.Button):
        if self.target_channel is None:
            embed = discord.Embed(description="Please select a channel first.", color=discord.Color.from_rgb(255, 255, 255))
            await interaction.response.send_message(embed=embed, ephemeral=True)
            return
        ...
        confirm_embed = discord.Embed(
            description=f"✅ Sent to {self.target_channel.mention}.", color=discord.Color.from_rgb(255, 255, 255)
        )
        await interaction.response.edit_message(embed=confirm_embed, view=None)
        if self.preview_message:
            await self.preview_message.edit(embed=confirm_embed, view=None)

    @discord.ui.button(label="Cancel", style=discord.ButtonStyle.secondary, emoji="✖️", row=3)
    async def cancel(self, interaction: discord.Interaction, button: discord.ui.Button):
        cancel_embed = discord.Embed(description="Cancelled.", color=discord.Color.from_rgb(255, 255, 255))
